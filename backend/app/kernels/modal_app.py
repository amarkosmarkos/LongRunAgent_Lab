...
# Modal needs one decorated function per GPU type; the bodies are identical.
for _gpu in GPUS:
    _fn = app.function(image=image, gpu=_gpu, timeout=900,
                       name=f"run_harness_{_gpu.lower()}")(
        lambda files, mode, seed=42, timeout_s=600: _run(
            files, mode, seed, timeout_s))
